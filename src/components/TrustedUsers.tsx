    ...
    >
      <div className="flex -space-x-4">
        {avatars.map((src, i) => (
          <div
            key={i}
            className={`w-10 h-10 rounded-full overflow-hidden ring-2 ring-offset-2 ring-offset-background ${
              ringColors[i] || "ring-primary"
            }`}
          >
            <img
              src={src}
      ...
      <div className="flex flex-col items-start gap-1">
        ...
        <span className="text-foreground text-xs md:text-sm font-medium">
          ...
          <CountUp
            ...
            duration={2}
            separator=","
            className="ml-1 text-sm md:text-base font-bold"
            suffix="+"
